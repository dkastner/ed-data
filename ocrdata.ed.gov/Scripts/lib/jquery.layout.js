...
,	getElemDims: function ($E) {
		...
		d.outerWidth	= $E.outerWidth(false);
		d.outerHeight	= $E.outerHeight(false);
...
,	isMouseOverElem: function (evt, el) {
		...
		,	T	= d.top
		,	L	= d.left
		,	R	= L + $E.outerWidth(false)
		,	B	= T + $E.outerHeight(false)
		,	x	= evt.pageX
		,	y	= evt.pageY
		;
 ...
 */
	...
	*/
	var _borderWidth = function (E, side) {
		...
		return $.curCSS(E, b+"Style", false) == "none" ? 0 : (parseInt($.curCSS(E, b+"Width", true), 10) || 0);
	};
	...
	*/
	var cssW = function (el, outerWidth) {
		...
		if (isNaN(outerWidth)) // not specified
			outerWidth = str ? getPaneSize(el) : $E.outerWidth(false);
	...
	*/
	var cssH = function (el, outerHeight) {
		...
		if (isNaN(outerHeight)) // not specified
			outerHeight = str ? getPaneSize(el) : $E.outerHeight(false);
	...
	*/
	var _parseSize = function (pane, size, dir) {
		...
		if (size === 0)
			...
			;
			$P.css(dim, "auto");
			size = (dim == "height") ? $P.outerHeight(false) : $P.outerWidth(false); // MEASURE
	...
	*/
	var getPaneSize = function (pane, inclSpace) {
		...
		if (!$P || s.isHidden)
			...
			return $P.outerHeight(false) + oSp;
		else // dir == "vert"
			return $P.outerWidth(false) + oSp;
	};
	...
	*/
	var setSizeLimits = function (pane, slide) {
		...
		,	altS			= state[altPane]
		,	$altP			= $Ps[altPane]
		,	altPaneSize		= (!$altP || altS.isVisible===false || altS.isSliding ? 0 : (dir=="horz" ? $altP.outerHeight(false) : $altP.outerWidth(false)))
	...
	*/
	var getElemDims = function ($E) {
        ...
        var offset = 0;
        if ($($E).hasClass('ui-layout-pane-west')) {
            offset = -2;
        }

		d.offsetWidth	= $E.innerWidth() + offset; // true=include Padding
		d.offsetHeight	= $E.innerHeight();
		d.outerWidth	= $E.outerWidth(false) + offset;
		d.outerHeight	= $E.outerHeight(false);
		d.innerWidth	= d.outerWidth  - i.Left - i.Right;
		d.innerHeight	= d.outerHeight - i.Top  - i.Bottom;

		// TESTING
		x.width  = $E.width() + offset;
		x.height = $E.height();
	...
	*/
	var lockPaneForFX = function (pane, doLock) {
		...
		if (doLock) {
			...
			if (pane=="south")
				$P.css({ top: sC.insetTop + sC.innerHeight - $P.outerHeight(false) });
			else if (pane=="east")
				$P.css({ left: sC.insetLeft + sC.innerWidth - $P.outerWidth(false) });
		}
		...
		$.each(panes.split(","), function (idx, pane) {
			...
			function _measure () {
				...
				if ($F.length)
					...
					m.spaceBelow = ($F[0].offsetTop + $F.outerHeight(false)) - m.bottom + _below($F);
	...
	var sizeHandles = function (panes) {
		...
		$.each(panes.split(","), function (i, pane) {
			...
			if (dir == "horz") { // north/south
				paneLen = $P.outerWidth(false); // s.outerWidth || 
				...
				paneLen = $P.outerHeight(false); // s.outerHeight || 
				s.resizerLength = paneLen;
				$R.css({
			...
			if ($T) {
				...
				if (dir == "horz") { // north/south
					...
					$T.children(".content").each(function(){
						$TC = $(this);
						$TC.css("marginLeft", Math.floor((width-$TC.outerWidth(false))/2)); // could be negative
					});
				}
				else { // east/west
					...
					$T.children(".content").each(function(){
						$TC = $(this);
						$TC.css("marginTop", Math.floor((height-$TC.outerHeight(false))/2)); // could be negative
					});
				}
